...
void solution(vector<star>s1,vector<star>s2,int*alpha,int*beta,int*want_sumx,int*want_sumy){
  vector<int>check;
  ...
  do{
    ...
    cout<<(sumx-*want_sumx)/s1.size()<<' '<<(sumy-*want_sumy)/s1.size()<<endl;
...
int main() {
  ...
  solution(want_star,sky_star,&place_x,&place_y,&want_sumx,&want_sumy);
